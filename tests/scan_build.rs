...
/// Run a full scan + build using the test harness.
fn run_scan_and_build(h: &TestHarness) -> Result<(Database, bob::BuildSummary)> {
    let config = h.load_config()?;
    let db = h.open_db()?;
    let state = h.run_state();
...
fn test_build_resume_no_new_work() -> Result<()> {
    ...
    let (_, first_build) = run_scan_and_build(&h)?;
...
fn test_rebuild_after_clear() -> Result<()> {
    ...
    // First build
    let (db, first) = run_scan_and_build(&h)?;
...
fn test_usergroup_phase_package() -> Result<()> {
    ...
    let mid = buildable
        ...
        .expect("mid should be in buildable set");
    assert_eq!(
        mid.usergroup_phase(),
        Some("configure"),
...
/// pkg_summary describes exactly the packages that built successfully:
/// each one appears, a package that failed does not, and every configured
/// compression format is written.
#[test]
fn test_pkg_summary_matches_successful_packages() -> Result<()> {
    let h = TestHarness::new()?;
    let (db, _) = run_scan_and_build(&h)?;
    ...
    assert!(
        !successful.is_empty(),
        "build should have produced packages"
    );
    ...
    assert_eq!(
        ...
        successful.len(),
        "one entry per successful package"
    );
    ...
    assert!(
        ...
        "pkg_summary should not contain a package that failed to build"
    );
